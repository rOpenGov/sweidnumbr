...
Up until now however, with the lack of a consistent R convention to handle "pins", the number might be treated as either a 10 or 12 digit numeric (with or without century prefix), a character (with hyphen or a '+'-sign to distinguish birth date from suffix numbers) or as a factor variable. But the pin is not a number (to add, subtract or logarithm pins is just nonsense) but is also contains more information than captured by the individual characters in a string. Luckily, the new R package `sweidnumbr` (released on CRAN) is now here for rescue!
...
```{r}
library(sweidnumbr)
knitr::kable(head(fake_swedes))
```

So far, pin is just a standard character vector but let's change that to benefit from all of `sweidnumbr`'s features:
```{r}
pin <- as.pin(fake_swedes$pin)
str(pin)
```


We can now also investigate some demographic characteristics almost on the fly:
```{r}
par(mfrow = c(1,2))
...
# Keep in touch!
... and feel free to suggest enhancements and report bugs to https://github.com/MansMeg/sweidnumbr/issues 