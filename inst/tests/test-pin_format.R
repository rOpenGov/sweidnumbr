...
test_that(desc="numeric: YYYYMMDDNNNC",{
  expect_equal(as.character(as.pin(196408233234)), expected = "196408233234")
  expect_equal(as.character(as.pin(200108230000)), expected = "200108230000")  
  expect_is(as.character(as.pin(200108230000)), "character")  
})

test_that(desc="numeric: YYMMDDNNNC",{
  expect_equal(as.character(as.pin(6408233234)), expected = "196408233234")  
  expect_equal(as.character(as.pin(108230000)), expected = "200108230000")  
  expect_equal(as.character(as.pin(pin = c(8230000,108230000))), expected = c("200008230000", "200108230000"))
})

test_that(desc="character: 'YYMMDDNNNC'",{
  expect_equal(as.character(as.pin("6408233234")), expected = "196408233234")  
  expect_equal(as.character(as.pin("0008230000")), expected = "200008230000")
  expect_equal(as.character(as.pin(today_pin)), expected = paste("20",today_pin, sep=""))  
})
...
test_that(desc="different formats",{
  expect_equal(as.character(as.pin(c("196408233234", "640823-3234", "19640823-3234", "6408233234"))), 
                          expected = rep("196408233234", 4))
  expect_equal(as.character(as.pin(c(196408233234, 6408233234))), 
